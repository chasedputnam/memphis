...
func TestKiroCLI_NoneCreatesMemphisAgent(t *testing.T) {
	ctx, dir := kiroCLIStore(t)
	...
	path := filepath.Join(dir, "memphis.json")
	if _, err := os.Stat(path); err != nil {
		t.Fatalf("memphis.json not created: %v", err)
	}
...
func TestKiroCLI_OneExistingMerged(t *testing.T) {
	...
	ptu := cliPostToolUse(t, path)
	if len(ptu) != 2 {
		t.Fatalf("expected 2 entries (existing + memphis), got %d", len(ptu))
	}
	if _, err := os.Stat(filepath.Join(dir, "memphis.json")); !os.IsNotExist(err) {
		t.Error("should not create memphis.json when one agent already exists")
	}
}
...
func TestKiroCLI_MultipleAmbiguous(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "memphis.json")); !os.IsNotExist(err) {
		t.Error("ambiguous install must not create memphis.json")
	}
...
func TestKiroCLI_KiroAgentSelects(t *testing.T) {
	...
	if len(cliPostToolUse(t, a)) != 1 {
		t.Error("selected agent a should get the memphis entry")
	}
	afterB, _ := os.ReadFile(b)
...
func TestKiroCLI_Idempotent(t *testing.T) {
	...
	if n := len(cliPostToolUse(t, filepath.Join(dir, "memphis.json"))); n != 1 {
		t.Errorf("re-install duplicated entry: %d", n)
	}
}

func TestKiroCLI_UninstallRemovesOnlyMemphis(t *testing.T) {
	ctx, dir := kiroCLIStore(t)
	...
	ptu := cliPostToolUse(t, path)
	if len(ptu) != 1 {
		t.Fatalf("expected only the non-memphis entry, got %d", len(ptu))
	}
	...
	if !strings.Contains(string(body), "echo r") {
		t.Error("non-memphis entry removed")
	}
}
